...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD calculateTravelID.
    ...
    SELECT SINGLE FROM zrap_atrav_mode
    ...
    MODIFY ENTITIES OF zi_rap_Travel_mode IN LOCAL MODE
    ENTITY Travel
    UPDATE FROM VALUE #( FOR travel IN travels INDEX INTO i (
        %tky              = travel-%tky
  ...
  METHOD validateAgency.
    ...
    IF agencies IS NOT INITIAL.
      ...
      WHERE agency_id EQ @agencies-agency_id
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky              = travel-%tky
                        %state_area       = 'VALIDATE_AGENCY'
                        %msg              = NEW zcm_rap_mode(
                                                severity = if_abap_behv_message=>severity-error
                                                textid = zcm_rap_mode=>agency_unknown
  ...
  METHOD validateCustomer.
    ...
    " Raise message for non-existing of initial customer id
    LOOP AT travels INTO DATA(travel).

      " Clear state messages that might exist
      APPEND VALUE #( %tky       = travel-%tky
                      %state_area = 'VALIDATE' ) TO reported-travel.
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-customerID ] ).
        ...
        APPEND VALUE #( %tky                = travel-%tky
                        ...
                        %msg                = NEW zcm_rap_mode(
                                                severity = if_abap_behv_message=>severity-error
                                                textid = zcm_rap_mode=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_mode(
                                                severity  = if_abap_behv_message=>severity-error
                                                textid    = zcm_rap_mode=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_mode(
                                                severity  = if_abap_behv_message=>severity-error
                                                textid    = zcm_rap_mode=>begin_date_before_system_date
                                                begindate = travel-BeginDate )
                        %element-BeginDate = if_abap_behv=>mk-on )
            TO reported-travel.
  ...
  METHOD get_instance_authorizations.
    ...
    " as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
    SELECT FROM zrap_atrav_mode
    ...
    is_delete_requested = COND #( WHEN requested_authorizations-%delete EQ if_abap_behv=>mk-on
                                  THEN abap_true ELSE abap_false ).
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      has_before_image = COND #( WHEN sy-subrc EQ 0 THEN abap_true ELSE AbaP_false ).

      IF is_update_requested = abap_true.

        IF has_before_image = abap_true." Edit of an existing record -> check update authorization
          ...
          IF update_granted = abap_false.
            ...
            APPEND VALUE #( %tky = travel-%tky
                            %msg = NEW zcm_rap_mode(
                                    severity = if_abap_behv_message=>severity-error
                                    textid   = zcm_rap_mode=>unauthorized ) )
            TO reported-travel.
  ...
  METHOD is_create_granted.
    ...
    AUTHORITY-CHECK OBJECT 'ZOSTATMODE'
      ID 'ZOSTATMODE' DUMMY
      ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
    ...
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTATMODE'
        ID 'ZOSTATMODE' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTATMODE'
        ID 'ZOSTATMODE' DUMMY
